...
{
	printf("Usage: %s [-v] [-h] \n",pro_name);
	printf("Usage: %s -r oldfile -w newfile -t type -o oriPara -n newPara\n",pro_name);
...
int ChangeMsg( u_char *Msg  , int MsgLen ) 
{
	...
	if( ( type& 1<<4) > 0  && atoi(oldpara) ==  HexToLong(Msg+34 , 2 ) )
	{
		MacToChar(newpara , Msg+6 )  ;	
	}
	
	if( ( type& 1<<5) > 0  && atoi(oldpara) ==  HexToLong(Msg+36 , 2 ) )
	{
		MacToChar(newpara , Msg+6 )  ;	
	}
}
