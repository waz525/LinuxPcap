...
int ParseOnePcap(struct pcap_pkthdr *m_headerPtr , const u_char *m_dataPtr)
{
	...
	//printf( "===> %lld --- %lld --- %d --- %d \n" , seqnum , acknum , lenData  , pDialog->seqFlag) ;
	int offset = 0 ;
	if( lenData > 0 )
	{
		...
		printf("    ===> %lld -- %lld -- %lld -- %lld -- %d -- %d -- %s -- %s -- %d \n" , seqnum , acknum , pDialog->initSeqNum , pDialog->initAckNum , offset , lenData , pDialog->IP1 , pDialog->IP2 , pDialog->seqFlag) ;
		int len = offset + lenData ;
		if( len > pDialog->len )
		{
	//		printf("    new==> %d --- %d --- %d --- %d\n" , offset , lenData , len , pDialog->len) ;
