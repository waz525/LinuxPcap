...
int main( int argc , char ** argv )
{
	...
	if( argc > 1 )
	{
		while((ch = getopt(argc,argv,"r:c:dvh*"))!= -1)
		{
			switch(ch)
			{
			case 'v':
	...
	printf("[INFO] Pcap File: %s ; max Dialog Count: %d ; Debug Mode: %d !\n" , pcapFile , maxDialog , isDebugMode ) ;
...
{
	printf("Usage: %s [-v] [-h] \n",pro_name);
	printf("Usage: %s -r PcapFile [-d] [-c maxDialogCount]\n",pro_name);
}
...
void WriteTCPDialog()
{
	...
	printf("[INFO] Parse %d Dialogs , Write %d files !\n" , countDialog , countFile ) ;
}
