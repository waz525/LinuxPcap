...
void ParseOnePcap(struct pcap_pkthdr *m_headerPtr , const u_char *m_dataPtr)
{
	...
	if( lenData > 0 )
	{
		...
		{
			memcpy(pDialog->content+offset , m_dataPtr+len_iphead+len_tcphead+LEN_ETHERNET , lenData ) ;
		}
	}
}
...
void WriteTCPDialog()
{
	...
	for( p=pHeader ; p!=NULL ; p=p->Next )
	{
		...
		if( isDebugMode == 1 ) printf( "[DEBUG] Index: %d , FileName: %s ; ContentLen: %d ; PcapCount: %d !\n" , p->index , filename , p->len , p->pcapcount) ;
		if( p->len > 0 )
		{
			FILE * pFile ;
