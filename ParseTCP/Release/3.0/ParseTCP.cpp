...
void WriteTCPDialog()
{
	...
	for( p=pHeader ; p!=NULL ; p=p->Next )
	{
		...
		printf( "[INFO] Dialog Index: %d , FileName: %s ; ContentLen: %d ; PcapCount: %d !\n" , p->index , filename , p->len , p->pcapcount) ;
		if( p->len > 0 )
		{
			FILE * pFile ;
			pFile = fopen( filename , "w" ) ;
			char * pchar = new char[p->len] ;
			bzero( pchar , p->len ) ;
			q=p->ContentLink ;
			while( q != NULL )
			{
				memcpy( pchar+q->offset , q->content , q->lenData ) ;
				q=q->Next ;
			}
			fwrite( pchar , p->len , 1 , pFile) ;
			fclose( pFile ) ;
			countFile++ ;
		}
	}
