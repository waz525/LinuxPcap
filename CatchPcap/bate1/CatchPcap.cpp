...
int main( int argc , char ** argv )
{
	...
	if( argc > 1 )
	{
		int ch;
		opterr = 0;
		while((ch = getopt(argc,argv,"i:f:w:c:r:pdvVhH?"))!= -1)
		{
			switch(ch)
			{
			case 'v':
...
{
        printf("Usage: %s [-i interface] [-f FILTER] [-c count] [-w file] [-r file] [-d] [-p] [-v] [-h] \n",pro_name);
        printf("options : \n");
        printf("        -v : show version \n");
        printf("        -h : show help \n");
        printf("        -i interface : set catch packets from which interface (used only for catch) \n");
        printf("        -f FILTER : set filter expression \n");
        printf("        -c count : set catch packet's count (used for catch or read ) \n");
        ...
        printf("        -d : whether to show detils of packet (used for catch or read )\n");
...
void AnalysePacket( struct pcap_pkthdr *m_headerPtr , const u_char *m_dataPtr ) 
{
	...
	{
	case ETHERTYPE_PUP:
		printf("ethertype PUP (0x0200), length %d: ",m_headerPtr->caplen ) ;
		break ;
		
	case ETHERTYPE_IP:
		printf("ethertype IPv4 (0x0800), length %d: ",m_headerPtr->caplen ) ;
		//src ip
	...
	case ETHERTYPE_ARP:
		printf("ethertype ARP (0x0806), length %d: ",m_headerPtr->caplen ) ;
		break ;
		
	case ETHERTYPE_REVARP:
		printf("ethertype REVARP (0x8035), length %d: ",m_headerPtr->caplen ) ;
		break ;
		
	defualt:
		printf("%s, length %d: ",str ,m_headerPtr->caplen ) ;		
	}
