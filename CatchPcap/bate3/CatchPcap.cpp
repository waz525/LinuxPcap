...
void AnalysePacket( struct pcap_pkthdr *m_headerPtr , const u_char *m_dataPtr ) 
{
	...
	case ETHERTYPE_IP:
		...
		case 6 :
			// TCP
			{
			if( m_dataPtr[47] & 1 ) printf("F ") ;
			else if( m_dataPtr[47] & 1<<1 ) printf("S ") ;
			else if( m_dataPtr[47] & 1<<2 ) printf("R ") ;
			else if( m_dataPtr[47] & 1<<3 ) printf("P ") ;
			else if( m_dataPtr[47] & 1<<4 ) printf("A ") ;
			
			int tcpDataLen = HexToLong(m_dataPtr+16,2) - (int)((m_dataPtr[14] & 0xf )<<2) - (int)(m_dataPtr[46]>>2) ;
			if( tcpDataLen == 0 ) tcpDataLen = 1 ;
			//printf("%lu:%lu(%d) ",HexToLong(m_dataPtr+38,4 ) , HexToLong(m_dataPtr+38,4 )+tcpDataLen , tcpDataLen) ;
			
			char temp[50] ;
			HexToString(m_dataPtr+38,4 , temp ) ;
			printf( "Sequence number : %s (%lu) ," , temp , HexToLong(m_dataPtr+38,4 ) ) ;
			HexToString(m_dataPtr+42,4 , temp ) ;
			printf( "Acknowledgement number : %s (%lu) , " , temp , HexToLong(m_dataPtr+42,4 )) ;
			
			printf( "Next sequence number : %lu " , HexToLong(m_dataPtr+38,4 )+tcpDataLen ) ;
			}
			break ;
		case 17 :
			// UDP
...
void HexToString(const unsigned char * hexChar , int length , char * newString )
{
	...
	for( i = 0 ; i < length ; i++ )
	{
		sprintf(newString+(i*2),"%02X", hexChar[i] ) ;
	}
	newString[i*2] = '\0' ;
